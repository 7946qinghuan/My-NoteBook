...
macOS 系统分为新旧两种默认终端 Shell 环境，不同环境对应的配置文件不同，配置逻辑、使用方法完全一致，仅文件名称有区别，新手无需额外修改代码逻辑： 
1. **新版 macOS（10.15 Catalina及以上）**：默认终端 Shell 为 **zsh**，对应配置文件为 `~/.zshrc`，也是本文全程使用的配置方式；
2. **旧版 macOS（10.14及以下）**：默认终端 Shell 为 **bash**，不识别 `.zshrc` 配置，需要使用专属配置文件 `~/.bash_profile`。
